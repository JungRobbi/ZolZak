...
D3D12_SHADER_BYTECODE BoundingShader::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSBounding", "vs_5_1", ppd3dShaderBlob));
}
D3D12_SHADER_BYTECODE BoundingShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSBounding", "ps_5_1", ppd3dShaderBlob));
}
...
D3D12_BLEND_DESC BoundingShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
...
D3D12_SHADER_BYTECODE BillboardShader::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSBillboard", "vs_5_1", ppd3dShaderBlob));
}
D3D12_SHADER_BYTECODE BillboardShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "PSBillboard", "ps_5_1", ppd3dShaderBlob));
}
...
void BillboardShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
...
D3D12_INPUT_LAYOUT_DESC ParticleShader::CreateInputLayout()
{
	...
	pd3dInputElementDescs[0] = { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[1] = { "VELOCITY", 0, DXGI_FORMAT_R32G32B32_FLOAT, 1, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[2] = { "COLOR", 0, DXGI_FORMAT_R32G32B32_FLOAT, 2, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[3] = { "EMITTIME", 0, DXGI_FORMAT_R32_FLOAT, 3, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[4] = { "LIFETIME", 0, DXGI_FORMAT_R32_FLOAT, 4, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
void ParticleShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	ID3DBlob* pd3dVertexShaderBlob = NULL, * pd3dPixelShaderBlob = NULL, * pd3dGeometryShaderBlob = NULL;
...
D3D12_SHADER_BYTECODE ParticleShader::CreateVertexShader(ID3DBlob** ppd3dShaderBlob)
{
	return(Shader::CompileShaderFromFile(L"Shaders.hlsl", "VSParticle", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE ParticleShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob)
{
...
//{
//	m_pToLightSpace = new TOOBJECTSPACEINFO;
//
...
//DepthRenderShader::~DepthRenderShader()
//{
//	if (m_pToLightSpace) delete m_pToLightSpace;
//}
//
...
//	m_pDepthTexture->CreateTexture(pd3dDevice, _DEPTH_BUFFER_WIDTH, _DEPTH_BUFFER_HEIGHT, DXGI_FORMAT_R32_FLOAT, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON, &d3dClearValue, RESOURCE_TEXTURE2D, 0);
//
//	D3D12_RENDER_TARGET_VIEW_DESC d3dRenderTargetViewDesc;
//	d3dRenderTargetViewDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;
//	d3dRenderTargetViewDesc.Texture2D.MipSlice = 0;
...
//
//	m_pDepthRenderCamera = new Camera();
//	m_pDepthRenderCamera->SetViewport(0, 0, _DEPTH_BUFFER_WIDTH, _DEPTH_BUFFER_HEIGHT, 0.0f, 1.0f);
//	m_pDepthRenderCamera->SetScissorRect(0, 0, _DEPTH_BUFFER_WIDTH, _DEPTH_BUFFER_HEIGHT);
//	m_pDepthRenderCamera->CreateShaderVariables(pd3dDevice, pd3dCommandList);
//
//
...
//{
//	if (m_pDepthRenderCamera)
//	{
//		m_pDepthRenderCamera->ReleaseShaderVariables();
//		delete m_pDepthRenderCamera;
//	}
//
...
//
//	ncbDepthElementBytes = ((sizeof(TOOBJECTSPACEINFO) + 255) & ~255); //256의 배수
//	m_pd3dcbToLightSpace = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbDepthElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
//
//	m_pd3dcbToLightSpace->Map(0, NULL, (void**)&m_pcbMappedToLightSpace);
//}
//
//void DepthRenderShader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
//{
//	::memcpy(m_pcbMappedToLightSpace, m_pToLightSpace, sizeof(TOOBJECTSPACEINFO));
//
//	D3D12_GPU_VIRTUAL_ADDRESS d3dcbToLightGpuVirtualAddress = m_pd3dcbToLightSpace->GetGPUVirtualAddress();
...
//{
//	if (m_pd3dcbToLightSpace)
//	{
//		m_pd3dcbToLightSpace->Unmap(0, NULL);
//		m_pd3dcbToLightSpace->Release();
//	}
//}
//
//void DepthRenderShader::PrepareShadowMap(ID3D12GraphicsCommandList* pd3dCommandList, LIGHT* light)
//{
//	XMFLOAT3 xmf3Position = light->m_xmf3Position;
//	XMFLOAT3 xmf3Look = light->m_xmf3Direction;
//	XMFLOAT3 xmf3Up = XMFLOAT3(0.0f, +1.0f, 0.0f);
//
//	XMMATRIX xmmtxView = XMMatrixLookToLH(XMLoadFloat3(&xmf3Position), XMLoadFloat3(&xmf3Look), XMLoadFloat3(&xmf3Up));
//
//	float fNearPlaneDistance = 10.0f, fFarPlaneDistance = light->m_fRange;
//
...
//	}
//	else if (light->m_nType == SPOT_LIGHT)
//	{
...
//	}
//
//	m_pDepthRenderCamera->SetPosition(xmf3Position);
//	XMStoreFloat4x4(&m_pDepthRenderCamera->m_xmf4x4View, xmmtxView);
//	XMStoreFloat4x4(&m_pDepthRenderCamera->m_xmf4x4Projection, xmmtxProjection);
//
//	XMMATRIX xmmtxToTexture = XMMatrixTranspose(xmmtxView * xmmtxProjection * m_xmProjectionToTexture);
//	XMStoreFloat4x4(&m_pToLightSpace->m_xmf4x4ToTexture, xmmtxToTexture);
//
//	m_pToLightSpace->m_xmf4Position = XMFLOAT4(xmf3Position.x, xmf3Position.y, xmf3Position.z, 1.0f);
//
//	::ResourceTransition(pd3dCommandList, m_pDepthTexture->GetResource(0), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_RENDER_TARGET);
//
...
//
//	Render(pd3dCommandList, m_pDepthRenderCamera);
//
//	::ResourceTransition(pd3dCommandList, m_pDepthTexture->GetResource(0), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COMMON);
//}
//
...
//
//	for (auto& obj : GameScene::MainScene->gameObjects)
//	{
//		obj->UpdateShaderVariables(pd3dCommandList);
//		obj->Render(pd3dCommandList, pCamera);
//	}
//	for (auto& obj : GameScene::MainScene->blendGameObjects)
//	{
//		obj->UpdateShaderVariables(pd3dCommandList);
//		obj->Render(pd3dCommandList, pCamera);
//	}
//	for (auto& obj : GameScene::MainScene->MonsterObjects)
//	{
//		obj->UpdateShaderVariables(pd3dCommandList);
//		obj->Render(pd3dCommandList, pCamera);
//	}
